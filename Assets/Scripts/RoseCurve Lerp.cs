...
public class RoseCurveLerp : MonoBehaviour
{
    GameObject[] spheres;
    ...
    void Start()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            float r = 3f;
            startPosition[i] = new Vector3(5f * Random.Range(-1f, 1f), 5f * Random.Range(-1f, 1f), 5f * Random.Range(-1f, 1f));        

            float theta = i * 2 * Mathf.PI / numSphere;
            // Circular end position
            endPosition[i] = new Vector3(
                r * (3f * Mathf.Cos(4f * theta)) * Mathf.Cos(theta),
                r * (3f * Mathf.Cos(4f * theta)) * Mathf.Sin(theta),
                10f);
        }
    ...
    void Update()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            Renderer sphereRenderer = spheres[i].GetComponent<Renderer>();
            float hue = (float)i / numSphere; // Hue cycles through 0 to 1
            Color color = Color.HSVToRGB(Mathf.Abs(hue * Mathf.Sin(time)), Mathf.Cos(time), 1f + 0.3f + Mathf.Cos(time)); // Full saturation and brightness
            sphereRenderer.material.color = color;
        }
    }
}
